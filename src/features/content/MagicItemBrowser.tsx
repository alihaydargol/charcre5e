 ...
 *
 * Bunlar tema jetonu değil, oyunun kendi nadirlik renkleri — yeşil uncommon,
 * mavi rare, mor very rare... Temayla dönmemeleri doğru; 500 tonları hem açık
 * hem koyu zeminde okunuyor.
 */
const RARITY_COLOR: Record<string, string> = {
  Common: 'bg-border-strong',
          ...
          {visible.map((item) => (
            <li key={item.id} className="overflow-hidden rounded-lg border border-border bg-surface">
              <button
                ...
                className="flex w-full items-start gap-3 p-4 text-left hover:bg-surface-muted"
              >
                <span
                  aria-hidden="true"
                  className={`mt-1 h-8 w-1 shrink-0 rounded-full ${RARITY_COLOR[item.rarity] ?? 'bg-border-strong'}`}
                />
              ...
              {openId === item.id && (
                <div className="space-y-2 border-t border-border px-4 py-3 text-sm leading-relaxed text-ink">
